...
const ThunderHoleTrackerMulti = () => {
  ...
  const LOCATION = {
    ...
    lat: 44.1539,
    lon: -68.4447,
    noaaStation: '8413825' // Mackerel Cove station (closest to Swan's Island)
  };
  ...
  const fetchNOAATideData = async () => {
    ...
    try {
      ...
      endDate.setDate(endDate.getDate() + 4); // Extended to 4 days to get more times
      
      const beginDate = today.toISOString().split('T')[0].replace(/-/g, '');
      const endDateStr = endDate.toISOString().split('T')[0].replace(/-/g, '');
      
      // NOAA API endpoint for tide predictions
      const url = `https://api.tidesandcurrents.noaa.gov/api/prod/datagetter?product=predictions&application=NOS.COOPS.TAC.WL&begin_date=${beginDate}&end_date=${endDateStr}&datum=MLLW&station=${LOCATION.noaaStation}&time_zone=lst_ldt&units=english&interval=hilo&format=json`;
  ...
  const calculateThunderHoleTimes = (tides: Tide[], maxTimes: number = 4): ThunderHoleTime[] => {
    const now = new Date();
    const thunderHoleTimes: ThunderHoleTime[] = [];
    ...
    // Find the best Thunder Hole times
    for (let tide of tides) {
      if (thunderHoleTimes.length >= maxTimes) break;
      ...
      const { sunrise, sunset } = getSunriseSunset(thunderHoleTime);
  ...
  const ThunderHoleTimeCard = ({ thunderTime, index }: { thunderTime: ThunderHoleTime; index: number }) => {
    ...
    return (
      <div className={`bg-gradient-to-br ${
        index === 0 
          ? 'from-orange-500/30 to-red-500/30 border-orange-400/50' 
          : 'from-orange-400/20 to-red-400/20 border-orange-300/30'
      } backdrop-blur-md rounded-xl p-6 border`}>
        <div className="text-center">
          {index === 0 && (
            <div className="bg-orange-500/20 rounded-full px-3 py-1 mb-3 inline-block">
              <span className="text-orange-200 text-xs font-semibold">NEXT BEST</span>
            </div>
          )}
          <p className="text-orange-200 text-sm mb-2">Optimal Time</p>
          <p className={`${index === 0 ? 'text-4xl' : 'text-3xl'} font-bold text-white mb-1`}>
            {thunderTime.time}
          </p>
          <p className="text-orange-200 text-sm mb-4">{getSmartDateLabel(thunderTime.timestamp)}</p>

          <div className="grid grid-cols-2 gap-3 text-center">
            <div>
              <p className="text-orange-200 text-xs mb-1">Time Remaining</p>
              <p className="text-lg font-bold text-white">{thunderTime.hoursFromNow.toFixed(1)}h</p>
            </div>
            <div>
              <p className="text-orange-200 text-xs mb-1">Related Tide</p>
              <p className="text-lg font-bold text-white">{thunderTime.relatedTide.type}</p>
              <p className="text-orange-200 text-xs">{thunderTime.relatedTide.time}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-800 via-blue-900 to-indigo-900 relative overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-blue-200">Find the perfect times to witness nature's power</p>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-r from-orange-600/20 to-red-600/20 backdrop-blur-md rounded-2xl p-8 border border-orange-400/40">
              <h2 className="text-3xl font-bold text-white mb-6 flex items-center gap-3">
                ...
                Best Times for Thunder Hole
              </h2>
              {(() => {
                const thunderHoleTimes = calculateThunderHoleTimes(tideData.tides, 4);
                if (thunderHoleTimes.length > 0) {
                  return (
                    <div className="space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4">
                        {thunderHoleTimes.map((thunderTime, index) => (
                          <ThunderHoleTimeCard 
                            key={index} 
                  ...
                  return (
                    <div className="bg-white/10 rounded-xl p-8 text-center">
                      ...
                      <p className="text-orange-300 text-sm">
                        Thunder Hole is best experienced during daylight for safety. The next optimal viewing times fall outside hiking hours.
                      </p>
                    </div>
                  );
                }
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {tideData.tides.slice(0, 9).map((tide, index) => (
...
export default ThunderHoleTrackerMulti;